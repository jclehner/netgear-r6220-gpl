...
#include <spi_api.h>
#include <nand_api.h>
#include <sc_cmd_def.h>

DECLARE_GLOBAL_DATA_PTR;
#undef DEBUG
#define MEM_CLEAR_VALUE 0x7000000
...
__attribute__((nomips16)) void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	u_boot_reloc_offset = gd->reloc_off;
...
*/
	...
	/* Copy Standalone Code into a DRAM postiion to cache it */
    memset(STANDALONE_COPY_IN_RAM_ADDR, 0xFF, STANDALONE_COPY_LENGTH);
    ranand_read((char *)STANDALONE_COPY_IN_RAM_ADDR, STANDALONE_FLASH_OFFSET, STANDALONE_LENGTH);
#endif
...
#ifdef _SC_BUILD_
		sc_standalone(SC_BOOT,0,1,NULL);
#endif

		printf("   \n3: System Boot system code via Flash.\n");
		printf("\n0x%x     0x%x         0x%x\n\n\n", CFG_KERN_ADDR, CFG_FLASH_BASE, CFG_KERN_ADDR - CFG_FLASH_BASE);
		eth_halt();
		printf("CLOSE ETH\nClear Memory\n");
		memset(CONFIG_KERNEL_LOAD_ADDR,0x0,MEM_CLEAR_VALUE);
		do_bootm(cmdtp, 0, 2, argv);
#ifdef _SC_BUILD_
		/* Kernel start up failed, enter sercomm upgrade */
		sc_standalone(SC_DL,0,1,NULL);
#endif
	}
	else {
		char *argv[4];
