...
#ifdef _SC_UPGRADE_
static unsigned int rx_buf[PKTSIZE_ALIGN + PKTALIGN];
unsigned char *sc_rx_buf;
void init_rx_buf(void)
{
	sc_rx_buf = rx_buf + (PKTALIGN - 1);
	sc_rx_buf -= (ulong)sc_rx_buf % PKTALIGN;
...
unsigned char *sc_rt2880_eth_recv(struct eth_device *dev, unsigned int *len)
{
	...
	memcpy(sc_rx_buf, (void *)KSEG1ADDR(NetRxPackets[rx_dma_owner_idx0]), length);
	//sc_rx_buf = (unsigned char *)KSEG1ADDR(NetRxPackets[rx_dma_owner_idx0]);
