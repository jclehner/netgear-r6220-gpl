...
#ifndef _SC_BUILD_
static int mtk_nand_write_oob(struct mtd_info *mtd, const uint8_t * buf, int page_addr, int len)
#else
...
#ifdef _SC_BUILD_
int mtk_nand_block_bad_hw(struct mtd_info *mtd, u32 ofs)
#else
static int mtk_nand_block_bad_hw(struct mtd_info *mtd, loff_t ofs)
#endif
{
    struct nand_chip *chip = (struct nand_chip *)mtd->priv;
...
#ifndef _SC_BUILD_ /*remove debug log*/
...
#ifdef _SC_BUILD_
/*
 * Sercomm add for standalone
 */
void get_nand_info(struct mtd_info *mtd , struct nand_chip *nand_chip)
{
	memcpy((unsigned char*)mtd, (unsigned char *)(&host->mtd) , sizeof(struct mtd_info));
	memcpy((unsigned char*)nand_chip , (unsigned char*)(&host->nand_chip),sizeof(struct nand_chip));
}
#endif
