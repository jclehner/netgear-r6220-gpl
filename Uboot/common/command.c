...
#endif
extern ulong u_boot_reloc_offset;
static int do_sc_mem_map (int flag, int argc, char *argv[])
{
	extern int _text_start, _text_end;
    extern int _rodata_start, _rodata_end;
    extern int _data_start, _data_end;
    extern int _gp;
    extern int _sdata_start, _sdata_end;
    extern int uboot_end_data, uboot_end;
    printf("**********************************************************\n");
    printf("%-25s:\t0x%x\n", "Memory Base", CONFIG_SYS_SDRAM_BASE);
    printf("%-25s:\t0x%x\n", "UBoot Text Start",  TEXT_BASE+u_boot_reloc_offset);
    printf("%-25s:\t0x%x\n", "UBoot Text End",  &_text_end);
    printf("%-25s:\t0x%x\n", "UBoot RODATA Start",  &_rodata_start);
    printf("%-25s:\t0x%x\n", "UBoot RODATA End",  &_rodata_end);
    printf("%-25s:\t0x%x\n", "UBoot DATA Start",  &_data_start);
    printf("%-25s:\t0x%x\n", "UBoot DATA End",  &_data_end);
    printf("%-25s:\t0x%x\n", "UBoot _gp",  &_gp);
    printf("%-25s:\t0x%x\n", "UBoot SDATA End",  &_sdata_end);
    printf("%-25s:\t0x%x\n", "UBoot End of Data",  &uboot_end_data);
    printf("%-25s:\t0x%x\n", "UBoot End",  &uboot_end);
    printf("%-25s:\t0x%x\n", "Kernel Load Addr",CONFIG_KERNEL_LOAD_ADDR);
    printf("**********************************************************\n");
	return 0;
}

U_BOOT_CMD(
	sc_mem_usage_info, 1,	1,	do_sc_mem_map,
	"sc_mem_usage_info  -SC: SC memory usage",
	""
);
