...
#########################################################################
#SeRcOmM
include $(TOPDIR)/sc_config.mk
#########################################################################
ifeq ($(SC_BUILD), y)
CFLAGS += -D_SC_BUILD_
CFLAGS += -I$(TOPDIR)/sc_include -I$(TOPDIR)/include/configs 
endif
ifeq ($(SC_STANDALONE),y)
SC_STANDALONE_DIR = sc_standalone
else
SC_STANDALONE_DIR = . 
endif
ifeq ($(SC_UPGRADE), y)
CFLAGS += -D_SC_UPGRADE_ -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_upgrade
endif
ifeq ($(SC_CMDSET), y)
CFLAGS += -D_SC_CMDSET_ -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_cmdset
endif

ifeq ($(SC_GPIO), y)
CFLAGS += -D_SC_GPIO_ -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_gpio
endif
ifeq ($(SC_MII), y)
CFLAGS += -D_SC_MII_

ifeq ($(SC_RAMTEST), y)
CFLAGS += -D_SC_RAMTEST_ -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_ramtest
endif
ifeq ($(SC_WATCHDOG), y)
CFLAGS += -D_SC_WDI_
endif
...
ifeq ($(SC_NAND_SUPPORT), y)
CFLAGS += -D_SC_NAND_SUPPORT_
endif
ifeq ($(SC_BMT_SUPPORT), y)
CFLAGS += -D_SC_BMT_SUPPORT_
endif
ifeq ($(SC_VERIFY_ECC), y)
CFLAGS += -D_SC_VERIFY_ECC_
endif
ifeq ($(SC_FINAL_BOOT), y)
CFLAGS += -D_SC_FINAL_BOOT_
endif
ifeq ($(SC_NMRP), y)
CFLAGS += -D_SC_NMRP_ 
CFLAGS += -D_SC_GP -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_include
CFLAGS += -D_SC_GP -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_common
CFLAGS += -D_SC_GP -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_nmrp
CFLAGS += -D_SC_GP -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_tftp
CFLAGS += -D_SC_GP -I$(TOPDIR)/$(SC_STANDALONE_DIR)/sc_tftp_rec
endif
ifeq ($(SC_TFTP_REC),y)
CFLAGS +=-D_SC_TFTP_REC_
endif
ifeq ($(SC_DUAL_IMAGE), y)
CFLAGS += -D_SC_DUAL_IMAGE_
endif
ifeq ($(SC_AMT), y)
CFLAGS += -D_SC_AMT_
endif
ifeq ($(SC_CHECKSUM), y)
CFLAGS += -D_SC_CHECKSUM_
endif
endif # endif SC_BUILD
